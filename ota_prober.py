...
class OTAProberGUI:
    def __init__(self, root):
        self.root = root
        self._root_main_thread = threading.get_ident()  # tkinter must be driven from here
        ...
        self._devbrute_speed = 0.0          # checks/sec
        self._devbrute_speed_ts = 0.0       # timestamp of last speed snapshot
        self._devbrute_speed_count = 0      # checks since last snapshot
        self._devbrute_wordlist_path = ""   # path to wordlist file (if set)
    ...
    def open_find_link_window(self):
        ...
        ttk.Label(frame, text="File name:", style='Normal.TLabel').grid(row=0, column=0, sticky=tk.W)
        self.findlink_name_var = tk.StringVar()
        name_entry = ttk.Entry(frame, textvariable=self.findlink_name_var, width=55)
        name_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=8)
        ttk.Button(frame, text="Check", command=self._findlink_start).grid(row=0, column=2)

        self.findlink_status_var = tk.StringVar(value="Enter a file name (e.g. 1234abcd....zip).")
        ttk.Label(frame, textvariable=self.findlink_status_var, style='Normal.TLabel',
                  wraplength=580, justify=tk.LEFT).grid(row=1, column=0, columnspan=3,
                                                        sticky=tk.W, pady=(10, 6))

        self.findlink_results = scrolledtext.ScrolledText(frame, height=14, wrap=tk.WORD)
        self.findlink_results.grid(row=2, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.findlink_results.configure(state=tk.DISABLED)

        frame.columnconfigure(1, weight=1)
        frame.rowconfigure(2, weight=1)
        self._findlink_win = win
        name_entry.focus_set()
        name_entry.bind('<Return>', lambda e: self._findlink_start())

    def _findlink_log(self, msg):
        try:
            w = self._findlink_win
            if not w or not w.winfo_exists():
                return
            self.findlink_results.configure(state=tk.NORMAL)
            self.findlink_results.insert(tk.END, msg + "\n")
            self.findlink_results.see(tk.END)
            self.findlink_results.configure(state=tk.DISABLED)
        except Exception:
            pass

    def _findlink_start(self):
        name = self.findlink_name_var.get().strip().strip('/')
        if not name:
            messagebox.showinfo("Find link", "Enter a file name first.")
            return
        self.findlink_results.configure(state=tk.NORMAL)
        self.findlink_results.delete('1.0', tk.END)
        self.findlink_results.configure(state=tk.DISABLED)
        self.findlink_status_var.set("Checking…")
        threading.Thread(target=self._findlink_worker, args=(name,), daemon=True).start()

    def _findlink_worker(self, name):
        prefixes = FIND_LINK_PREFIXES or [""]
        urls = [prefix + name for prefix in prefixes]
        ...
        for url in urls:
            def status_cb(m):
                self.root.after(0, self.findlink_status_var.set, m)
            try:
                res = probe_ota_url(url, status_cb=status_cb, timeout=15)
                status_line = res.get('summary') or ''
                ok = 'HTTP 200' in status_line
            except Exception as exc:
                status_line = f"Error: {exc}"
                ok = False
            if ok:
                found.append(url)
            line = ("[OK]      " if ok else "[failed]  ") + url
            if not ok:
                line += f"   ({status_line})"
            self.root.after(0, self._findlink_log, line)

        def done():
            if found:
                self.findlink_status_var.set(f"{len(found)} working link(s) found.")
                for u in found:
                    self._findlink_log("→ " + u)
            else:
                self.findlink_status_var.set("No working links found.")
    ...
    def create_widgets(self):
        ...
        self.find_link_button = ttk.Button(
            history_frame, text="Find link", command=self.open_find_link_window)
        self.find_link_button.pack(side=tk.RIGHT, padx=(6, 0))

        # Board preset and App ID vars — widgets placed below next to Copy link
        ...
        self._cros_appid_row_widgets = []  # appid combo now in header_frame, handled via pack
        self._cros_only_widgets = []  # board/appid are now in header_frame, handled via pack
        ...
        # Unified stop button — stops whichever scan is currently running
        ...
        # ChromeOS Board preset + App ID — right of Copy to Clipboard, hidden when not ChromeOS
        ...
        self.cros_board_label = ttk.Label(self.cros_header_frame, text="")  # kept for _cros_only_widgets compat
        ...
        self.cros_appid_label = ttk.Label(self.cros_header_frame, text="")  # kept for _cros_only_widgets compat
        ...
        # ── Play Games Emulator editor (shown only in playemu mode) ─────────
        ...
        # Status bar with dogfood indicator
        ...
        # Bruteforce tab (combined with serial brute)
    ...
    def _update_input_frame_visibility(self):
        """Show or hide the Device Fingerprint frame.
        In playemu mode it is always hidden (the editor replaces it).
        In all other modes it is always visible.
        """
        mode = self.os_mode_var.get()
        if mode == "playemu":
    ...
    def _set_os_mode_visibility(self):
        ...
        # Show/hide Play Games Emulator inline editor
        ...
        # Hide Bruteforce / URL Bruteforce / Device Bruteforce tabs in playemu mode
        _brute_tab_pairs = [
        ...
        for attr, label in _brute_tab_pairs:
            ...
            if is_playemu:
                ...
                if not present:
                    # Insert before Send Raw Request to preserve tab order
                    try:
                        rawreq_idx = next(
        ...
        # Show/hide Board preset + App ID frame (ChromeOS only) — lives in button_frame after Copy to Clipboard
        ...
        # Show/hide the "Scan Key Types" / "OTA Chain Check" / "⏹ Stop Scan"
        # buttons (Android only — not ChromeOS or Xiaomi)
        ...
        # Hide the whole Device Fingerprint frame in playemu mode,
        # but only when not on a tab where it still makes sense (HTTP Info, Send Raw Request)
        ...
        # Show/hide Android-only toolbar buttons
        _android_only_btns = [
            'download_ota_button',
            'find_link_button',
    ...
    def _raw_save_gzip(self):
        """Зберігає сирий gzip-відповідь сервера у файл."""
    ...
    def _raw_request_populate(self):
        """Заповнює Raw Request вкладку після query."""
        req = getattr(self, '_last_request_data', None)
        if not req:
            return
        # Detect content type to pick the right human-readable formatter
    ...
    def _raw_req_save(self, mode: str):
    ...
    def _build_bruteforce_tab(self):
        ...
        self.brute_inc_pad_var = tk.StringVar(value="0")
        ...
        ttk.Label(self.brute_inc_lf, text="Pad = zero-filled width (0 = off)",
                  foreground='#888888').grid(row=4, column=0, columnspan=2, sticky=tk.W)

        # ── Serial Bruteforce section ──────────────────────────────────────
        ...
        # Num1 row
        _num1_row = ttk.Frame(self.brute_serial_frame)
        _num1_row.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(_num1_row, text="{num1}:", width=9, font=('Courier', 9, 'bold')).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Label(_num1_row, text="Start:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_start_var = tk.StringVar(value="1")
        ttk.Entry(_num1_row, textvariable=self.brute_serial_start_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num1_row, text="End:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_end_var = tk.StringVar(value="9999")
        ttk.Entry(_num1_row, textvariable=self.brute_serial_end_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num1_row, text="Step:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_step_var = tk.StringVar(value="1")
        ttk.Entry(_num1_row, textvariable=self.brute_serial_step_var, width=6).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(_num1_row, text="Pad:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_pad_var = tk.StringVar(value="0")
        ttk.Entry(_num1_row, textvariable=self.brute_serial_pad_var, width=4).pack(side=tk.LEFT, padx=(0, 12))
        self.brute_serial_hex_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num1_row, text="HEX", variable=self.brute_serial_hex_var).pack(side=tk.LEFT)
        self.brute_serial_base36_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num1_row, text="Base36", variable=self.brute_serial_base36_var).pack(side=tk.LEFT, padx=(8, 0))

        # Num2 row
        _num2_row = ttk.Frame(self.brute_serial_frame)
        _num2_row.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(_num2_row, text="{num2}:", width=9, font=('Courier', 9, 'bold')).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Label(_num2_row, text="Start:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_start2_var = tk.StringVar(value="1")
        ttk.Entry(_num2_row, textvariable=self.brute_serial_start2_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num2_row, text="End:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_end2_var = tk.StringVar(value="9999")
        ttk.Entry(_num2_row, textvariable=self.brute_serial_end2_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num2_row, text="Step:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_step2_var = tk.StringVar(value="1")
        ttk.Entry(_num2_row, textvariable=self.brute_serial_step2_var, width=6).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(_num2_row, text="Pad:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_pad2_var = tk.StringVar(value="0")
        ttk.Entry(_num2_row, textvariable=self.brute_serial_pad2_var, width=4).pack(side=tk.LEFT, padx=(0, 12))
        self.brute_serial_hex2_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num2_row, text="HEX", variable=self.brute_serial_hex2_var).pack(side=tk.LEFT)
        self.brute_serial_base36_2_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num2_row, text="Base36", variable=self.brute_serial_base36_2_var).pack(side=tk.LEFT, padx=(8, 0))

        # Num3 row
        _num3_row = ttk.Frame(self.brute_serial_frame)
        _num3_row.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(_num3_row, text="{num3}:", width=9, font=('Courier', 9, 'bold')).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Label(_num3_row, text="Start:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_start3_var = tk.StringVar(value="1")
        ttk.Entry(_num3_row, textvariable=self.brute_serial_start3_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num3_row, text="End:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_end3_var = tk.StringVar(value="9999")
        ttk.Entry(_num3_row, textvariable=self.brute_serial_end3_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num3_row, text="Step:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_step3_var = tk.StringVar(value="1")
        ttk.Entry(_num3_row, textvariable=self.brute_serial_step3_var, width=6).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(_num3_row, text="Pad:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_pad3_var = tk.StringVar(value="0")
        ttk.Entry(_num3_row, textvariable=self.brute_serial_pad3_var, width=4).pack(side=tk.LEFT, padx=(0, 12))
        self.brute_serial_hex3_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num3_row, text="HEX", variable=self.brute_serial_hex3_var).pack(side=tk.LEFT)
        self.brute_serial_base36_3_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num3_row, text="Base36", variable=self.brute_serial_base36_3_var).pack(side=tk.LEFT, padx=(8, 0))

        # Num4 row
        _num4_row = ttk.Frame(self.brute_serial_frame)
        _num4_row.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(_num4_row, text="{num4}:", width=9, font=('Courier', 9, 'bold')).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Label(_num4_row, text="Start:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_start4_var = tk.StringVar(value="1")
        ttk.Entry(_num4_row, textvariable=self.brute_serial_start4_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num4_row, text="End:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_end4_var = tk.StringVar(value="9999")
        ttk.Entry(_num4_row, textvariable=self.brute_serial_end4_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num4_row, text="Step:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_step4_var = tk.StringVar(value="1")
        ttk.Entry(_num4_row, textvariable=self.brute_serial_step4_var, width=6).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(_num4_row, text="Pad:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_pad4_var = tk.StringVar(value="0")
        ttk.Entry(_num4_row, textvariable=self.brute_serial_pad4_var, width=4).pack(side=tk.LEFT, padx=(0, 12))
        self.brute_serial_hex4_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num4_row, text="HEX", variable=self.brute_serial_hex4_var).pack(side=tk.LEFT)
        self.brute_serial_base36_4_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num4_row, text="Base36", variable=self.brute_serial_base36_4_var).pack(side=tk.LEFT, padx=(8, 0))

        # Num5 row
        _num5_row = ttk.Frame(self.brute_serial_frame)
        _num5_row.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(_num5_row, text="{num5}:", width=9, font=('Courier', 9, 'bold')).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Label(_num5_row, text="Start:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_start5_var = tk.StringVar(value="1")
        ttk.Entry(_num5_row, textvariable=self.brute_serial_start5_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num5_row, text="End:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_end5_var = tk.StringVar(value="9999")
        ttk.Entry(_num5_row, textvariable=self.brute_serial_end5_var, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(_num5_row, text="Step:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_step5_var = tk.StringVar(value="1")
        ttk.Entry(_num5_row, textvariable=self.brute_serial_step5_var, width=6).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Label(_num5_row, text="Pad:").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_serial_pad5_var = tk.StringVar(value="0")
        ttk.Entry(_num5_row, textvariable=self.brute_serial_pad5_var, width=4).pack(side=tk.LEFT, padx=(0, 12))
        self.brute_serial_hex5_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num5_row, text="HEX", variable=self.brute_serial_hex5_var).pack(side=tk.LEFT)
        self.brute_serial_base36_5_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(_num5_row, text="Base36", variable=self.brute_serial_base36_5_var).pack(side=tk.LEFT, padx=(8, 0))

        ttk.Label(serial_section,
                  text="Pad = fixed zero-padded width for the value (0 = auto), e.g. Pad 10 → 0000000001, 0000000002 …",
                  foreground="#666666").grid(row=2, column=0, columnspan=2, sticky=tk.W)

        # ── IMEI Bruteforce section ──────────────────────────────────────────────────────
        imei_section = ttk.LabelFrame(wrapper, text="IMEI Bruteforce", padding="6")
        imei_section.pack(fill=tk.X, pady=(6, 0))

        # Row 0: enable checkbox + TAC field
        ...
        # Row 1: SNR range + step + zero-pad toggle
        ...
        # ── HTC Serial Bruteforce section ────────────────────────────────────
        htc_section = ttk.LabelFrame(wrapper, text="HTC Serial Bruteforce  (<FC><Y><M><C><DD><SSSSS>)", padding="6")
        ...
        ttk.Label(htc_row0, text="Factory code (FC):").pack(side=tk.LEFT, padx=(0, 4))
        self.brute_htc_factory_var = tk.StringVar(value="HT")
        ttk.Entry(htc_row0, textvariable=self.brute_htc_factory_var, width=6,
                  font=("Courier", 9)).pack(side=tk.LEFT, padx=(0, 16))
        ...
        ttk.Label(htc_section,
                  text="Format: FC(factory code, e.g. HT/FA/TY) + Y(year last digit) + M(month: 1-9,A,B,C) + C(unknown) + DD(device) + SSSSS(seq 0-99999).  "
                       "Known DD codes: U11=1A  U11+=1B  U12+=1C  OneM7=01  OneM8=02  OneM9=03  OneA9=0A  HTC10=10.  "
                       "Factory code can be any string (leave as HT for standard HTC serials).",
                  foreground="#666666", wraplength=900, justify=tk.LEFT).pack(anchor=tk.W, pady=(2, 0))

        # ── Pixel Date-based Serial Bruteforce ───────────────────────────────
    ...
    def _brute_start(self):
        ...
        if is_cros:
            ...
            try:
                inc_pad = int(self.brute_inc_pad_var.get().strip())
            except ValueError:
                inc_pad = 0
            if inc_pad < 0:
                inc_pad = 0
            ...
            if use_ver and inc_pad:
                self._brute_log(f"Version zero-pad: {inc_pad} digits", 'header')
            if serial_enabled:
            ...
            self._brute_producer_thread = threading.Thread(
                ...
                args=(board_tags, tracks, inc_start, inc_end, inc_step, template, n_workers,
                      use_board, use_ver, use_track, app_id, serial_enabled, inc_pad),
                daemon=True
            )
        ...
        self._brute_dogfood_count = 0
        # Cache UI vars once — worker threads must never call tkinter .get() per-hit
        ...
        if use_inc and inc_pad:
            self._brute_log(f"Incremental zero-pad: {inc_pad} digits", 'header')
        if serial_enabled:
            self._brute_log("Serial bruteforce enabled.", 'header')
        if imei_enabled:
        ...
        self._brute_resume_skip = 0  # reset so next normal Start doesn't skip
        self._brute_producer_thread = threading.Thread(
            target=self._brute_producer,
            args=(build_tags, key_types, inc_start, inc_end, inc_step, template, n_workers,
                  use_build, use_inc, use_key, locales, serial_enabled, _skip, inc_pad),
            daemon=True
        )
        self._brute_producer_thread.start()
        if _skip > 0:
    ...
    # ── IMEI Bruteforce helpers ──────────────────────────────────────────────
    @staticmethod
    def _luhn_check_digit(fourteen_digits):
        """Compute Luhn check digit for a 14-digit string, returning full 15-digit IMEI."""
    ...
    def _generate_imeis(self):
        """Lazily yield valid IMEI strings based on UI settings (no list built in RAM)."""
        ...
        for snr in range(snr_start, snr_end + 1, snr_step):
            ...
            fourteen = tac + snr_str[-6:]  # take last 6 chars in case user put >6 digit SNR
    ...
    def _compute_serial_count(self):
        """Compute total serial combinations: num1_count × num2_count × … (only for active placeholders)."""
    ...
    def _brute_producer(self, build_tags, key_types, inc_start, inc_end, inc_step, template, n_workers,
                        use_build, use_inc, use_key, locales, serial_enabled, skip_count=0, inc_pad=0):
        try:
            ...
            # Flat combo generator — yields one tuple at a time, zero lists in RAM.
            def _sn_sequence():
                """Yield all active_sn values as a flat sequence (alternatives, not products)."""
                htc_enabled   = self.brute_htc_enabled_var.get()
                pixel_enabled = self.brute_pixel_enabled_var.get()
                ...
                if moto_on:
                    for v in self._generate_moto_serials():
                        if v is not None:
                            yield v
                # If nothing enabled, yield a single None so the loop still runs
                            ...
                            for loc in locales:
                                for imei_val in self._generate_imeis():
                                    for active_sn in _sn_sequence():
                                        yield (bt, kt,
                                               str(inc).zfill(inc_pad) if use_inc else "",
            ...
            for combo in _all_combos():
                ...
                if skipped < skip_count:
                    skipped += 1
                    continue
                # Non-blocking put with stop-check so UI stays responsive
                if self._brute_stop_flag:
                    return
    ...
    def _generate_htc_serials(self):
        ...
        try:
            ...
            if seq_step <= 0: seq_step = 1
            if seq_start > seq_end: seq_start, seq_end = seq_end, seq_start
        except ValueError:
            yield None
            return
        ...
        for y, m, c, dd in _itertools.product(years, months, uchars, dcodes):
            for seq in range(seq_start, seq_end + 1, seq_step):
                yield f"{factory}{y}{m}{c}{dd}{seq:05d}"
                yielded = True
        if not yielded:
            yield None
    ...
    # ── Pixel date-based Serial generator ───────────────────────────────────────
    def _generate_pixel_serials(self):
        ...
        try:
            ...
            if seq_step <= 0: seq_step = 1
            if day_start > day_end: day_start, day_end = day_end, day_start
            if seq_start > seq_end: seq_start, seq_end = seq_end, seq_start
        except ValueError:
            yield None
            return
    ...
    # ── Motorola Serial generator ────────────────────────────────────────────────
    def _generate_moto_serials(self):
        """Yield Motorola serials: P02 + R + FF + seq (alphanumeric, indexed)."""
    ...
    # ── Stop + Save / Load Progress ─────────────────────────────────────────────
    def _brute_stop_and_save(self):
        """Stop bruteforce and save all settings + current progress to a JSON file."""
        self._brute_stop()
        path = filedialog.asksaveasfilename(
        ...
        data = {
            ...
            "progress": {
                ...
                "total":     total,
                "found":     found,
                # skip_count tells producer how many combos to skip on resume
            ...
            "settings": {
                ...
                "inc_pad":            _safe(self.brute_inc_pad_var),
                ...
                "serial_pad":         _safe(self.brute_serial_pad_var),
                ...
                "serial_pad2":        _safe(self.brute_serial_pad2_var),
                ...
                "serial_pad3":        _safe(self.brute_serial_pad3_var),
                ...
                "serial_pad4":        _safe(self.brute_serial_pad4_var),
                ...
                "serial_pad5":        _safe(self.brute_serial_pad5_var),
    ...
    def _brute_load_progress(self):
        """Load settings from a previously saved progress JSON and restore all fields."""
        path = filedialog.askopenfilename(
        ...
        _set(self.brute_inc_pad_var,           "inc_pad", "0")
        ...
        _set(self.brute_serial_pad_var,        "serial_pad", "0")
        ...
        _set(self.brute_serial_pad2_var,       "serial_pad2", "0")
        ...
        _set(self.brute_serial_pad3_var,       "serial_pad3", "0")
        ...
        _set(self.brute_serial_pad4_var,       "serial_pad4", "0")
        ...
        _set(self.brute_serial_pad5_var,       "serial_pad5", "0")
        ...
        skip_count = prog.get("skip_count", processed)  # v2 field

        # Store skip count so _brute_start passes it to producer
    ...
    def _generate_serials(self):
        if not self.brute_serial_enabled_var.get():
            return
        template = self.brute_serial_template_var.get().strip()
        # {num} is treated as {num1} for backward compatibility
        has_num1 = '{num1}' in template or '{num}' in template
        has_num2 = '{num2}' in template
        has_num3 = '{num3}' in template
        has_num4 = '{num4}' in template
        ...
        def _parse_pad(pad_var):
            """Parse a Pad field: >0 = fixed zero-padded width, otherwise 0 (auto)."""
            try:
                p = int((pad_var.get() or "").strip())
            except (ValueError, AttributeError):
                return 0
            return p if p > 0 else 0

        def _make_formatter(start_var, end_var, step_var, hex_var, b36_var, pad_var=None):
            """Return (start, end, step, fmt_func) or None on parse error."""
            try:
                start = int(start_var.get().strip())
                end   = int(end_var.get().strip())
                raw_s = step_var.get().strip()
                ...
                return None
            pad = _parse_pad(pad_var) if pad_var is not None else 0
            if b36_var.get():
                width = pad if pad else max(len(_b36(start, 1)), len(_b36(end, 1)))
                fmt = lambda n, w=width: _b36(n, w)
            elif hex_var.get():
                # HEX mode — uppercase letters
                width = pad if pad else max(len(f"{start:X}"), len(f"{end:X}"))
                fmt = lambda n, w=width: f"{n:0{w}X}"
            else:
                width = pad if pad else max(len(str(start)), len(str(end)))
        ...
        cfg1 = _make_formatter(
            ...
            self.brute_serial_base36_var, self.brute_serial_pad_var) if has_num1 else None
        ...
        cfg2 = _make_formatter(
            ...
            self.brute_serial_base36_2_var, self.brute_serial_pad2_var) if has_num2 else None
        ...
        cfg3 = _make_formatter(
            ...
            self.brute_serial_base36_3_var, self.brute_serial_pad3_var) if has_num3 else None
        ...
        cfg4 = _make_formatter(
            ...
            self.brute_serial_base36_4_var, self.brute_serial_pad4_var) if has_num4 else None
        ...
        cfg5 = _make_formatter(
            ...
            self.brute_serial_base36_5_var, self.brute_serial_pad5_var) if has_num5 else None
        ...
        # Build list of (placeholder, cfg) pairs for active nums
        active = []
        if has_num1: active.append(('{num1}', cfg1))
        if has_num2: active.append(('{num2}', cfg2))
        if has_num3: active.append(('{num3}', cfg3))
        if has_num4: active.append(('{num4}', cfg4))
        if has_num5: active.append(('{num5}', cfg5))
    ...
    def _brute_worker(self, device_sn="", imei="", serial_enabled=False):
        while True:
            ...
            # Use serial if provided, else device_sn
            actual_sn = serial if serial is not None else device_sn
            # Use bruteforced IMEI if provided, else fall back to the static IMEI field
                ...
                try:
                    ...
                    if not settings:
                        ...
                        if hard_attempt >= max_hard_retries:
                            _lparts = []
                            if build_tag: _lparts.append(f"BUILD={build_tag}")
                            if key_type:  _lparts.append(f"KEY={key_type}")
                            if inc:       _lparts.append(f"INC={inc}")
                            if loc:       _lparts.append(f"LOCALE={loc}")
                            label = " ".join(_lparts)
                    ...
                    if is_network:
                        # transient network error — retry immediately, no limit
                        continue
                    hard_attempt += 1
                    if hard_attempt >= max_hard_retries:
                        _lparts = []
    ...
    def _brute_process_result(self, fp, build_tag, key_type, inc, loc, ota, raw_bytes, serial=None, imei_val=None):
        # Use cached values — never call tkinter .get() from a worker thread
        ...
        # Dogfood detection - only droidfood or platform_dogfood
        is_dogfood = False
        if raw_bytes:
        ...
        if self.os_mode_var.get() == "xiaomi":
            ...
            _lparts = []
            if build_tag:
                _lparts.append(f"BUILD={build_tag}")
            if key_type:
                _lparts.append(f"KEY={key_type}")
            if inc:
                _lparts.append(f"INC={inc}")
            if loc:
                _lparts.append(f"LOCALE={loc}")
            label = " ".join(_lparts)
            if serial is not None:
                label += f" SERIAL={serial}"
            if imei_val is not None:
        ...
        # ── Single lock acquisition: check duplicate AND update counters together ──
        with self._brute_progress_lock:
            ...
            if meta_set is None:
                ...
                is_changed = True
                is_duplicate = False
            # Count every hit (including dupes) under the same lock
            self._brute_found_count += 1
            if is_dogfood:
                self._brute_dogfood_count += 1
            local_count = len(self._brute_found_data)

        # ── Fast path: duplicate — skip expensive file I/O entirely ──────────────
        if is_duplicate:
            # Always log dogfood serials regardless of duplicate status
            if is_dogfood:
                serial_str = f": {serial}" if serial is not None else ""
            ...
            return  # always skip file I/O for dupes; log controlled by hide_dupes_log

        # ── Only NEW or CHANGED hits reach here — now do file I/O ────────────────
        try:
            add_ota_record(
                os_kind=self.os_mode_var.get(),
                url=url,
        ...
        # Dogfood serials are logged in the brute log and saved to OTAs.txt (see [DOGFOOD] tag above)

        # Pause only on new OTA (not updates)
    ...
    def _brute_update_speed(self):
        """Periodically refresh the req/s counter for the main Bruteforce tab."""
        if not self._brute_running:
    ...
    # ── ChromeOS bruteforce producer/worker (with serial support) ───────────────
    def _brute_producer_chromeos(self, board_tags, tracks, inc_start, inc_end, inc_step, template, n_workers,
                                  use_board, use_ver, use_track, app_id, serial_enabled, inc_pad=0):
        try:
                    ...
                    for inc in inc_values:
                        for tr in trks:
                            for serial in (self._generate_serials() if serial_enabled else (None,)):
                                yield (bt, tr, str(inc).zfill(inc_pad) if use_ver else "", template, app_id, serial)
    ...
    def _brute_worker_chromeos(self, serial_enabled=False):
        while True:
            ...
            # ChromeOS checkin does not use serial directly, but we can pass it for logging
            # The serial is not used in the checkin request, but we can still detect dogfood
            # from the raw response (though ChromeOS responses are XML, not protobuf).
            # We'll still log serial if provided.
    ...
    # ── Xiaomi bruteforce start/producer/worker (with serial support) ───────────────────────────
    def _brute_start_xiaomi(self, template, serial_enabled):
        ...
        try:
            and_pad = int(self.brute_inc_pad_var.get().strip())
        except ValueError:
            and_pad = 0
        if and_pad < 0:
            and_pad = 0
        ...
        self._brute_log(f"Template: {template}", 'header')
        if use_and and and_pad:
            self._brute_log(f"Android version zero-pad: {and_pad} digits", 'header')
        if serial_enabled:
            self._brute_log("Serial bruteforce enabled.", 'header')
        self._brute_log("=" * 70, 'header')
        ...
        self._brute_producer_thread = threading.Thread(
            ...
            args=(devices, roms, and_start, and_end, and_step, template, n_workers,
                  use_device, use_rom, use_and, serial_enabled, and_pad),
            daemon=True
        )
        self._brute_producer_thread.start()
    ...
    def _brute_producer_xiaomi(self, devices, roms, and_start, and_end, and_step, template, n_workers,
                                use_device, use_rom, use_and, serial_enabled, and_pad=0):
        try:
            ...
            def _all_combos_xiaomi():
                for dv in devs:
                    for av in and_values:
                        for rv in rom_list:
                            for serial in (self._generate_serials() if serial_enabled else (None,)):
                                yield (dv, rv, str(av).zfill(and_pad) if use_and else "", template, serial)
    ...
    def _build_urlbrute_tab(self):
        ...
        # ── Options (workers + success codes — всe інше автоматичне) ─────────
        opt_lf = ttk.LabelFrame(wrapper, text="Options", padding="6")
        ...
        # ── Wordlist Mode (replaces charset bruteforce when a file is loaded) ───
    ...
    def _ub_open_log(self):
        ...
        frame = ttk.Frame(win, padding="10")
        frame.pack(fill=tk.BOTH, expand=True)
        tb = ttk.Frame(frame); tb.pack(fill=tk.X, pady=(0,6))
    ...
    # ── Wordlist helpers (URL Bruteforce) ────────────────────────────────────
    ...
    def _ub_wordlist_iter(self, path):
        """Generator: yields non-empty stripped lines from a text file in chunks."""
        try:
            with open(path, "r", encoding="utf-8", errors="replace") as fh:
                leftover = ""
                while True:
    ...
    def _ub_pause(self):
        if not self._ub_running: return
    ...
    def _ub_finish(self, stop=False):
        ...
        self.ub_continue_btn.config(state=tk.DISABLED)
        self.ub_stop_btn.config(state=tk.DISABLED)
        # Stop indeterminate progress bar if it was running in wordlist mode
        try:
    ...
    # ── Monitor (main-thread polling, як в основному brute) ───────────────────
    ...
    def _ub_update_speed(self):
        """Periodically refresh the req/s counter for URL Bruteforce."""
    ...
    def _ub_start(self):
        ...
        try: n_workers = max(1, int(self.ub_workers_var.get()))
        except ValueError: n_workers = 10
        try: timeout = max(1, int(self.ub_timeout_var.get()))
        except ValueError: timeout = 10
        try: fail_codes = {int(c.strip()) for c in self.ub_codes_var.get().split(',') if c.strip()}
        except ValueError: fail_codes = {404}
        ...
        if use_wordlist:
            total = 0  # unknown until wordlist is streamed; indeterminate progress
        elif fixed:
    ...
    def _ub_producer_wordlist(self, wordlist_path, templates, n_workers):
        """Wordlist producer for URL Bruteforce — streams lines from a .txt file."""
        try:
    ...
    def _ub_worker(self, timeout, fail_codes, stop_on_find, hide_no_ota=False, hide_dupes=False):
        ...
        while True:
            ...
            if self._ub_stop_flag:
                # drain
                try:
                    while True: self._ub_queue.get_nowait()
                except queue.Empty:
                    pass
                break
            ...
            # після взяття item — знову перевіряємо паузу
            self._ub_pause_event.wait()
            if self._ub_stop_flag:
                try:
                    while True: self._ub_queue.get_nowait()
            ...
            # Retry loop — тільки на мережеві помилки, як в основному brute
            while True:
                if self._ub_stop_flag:
                    break
                self._ub_pause_event.wait()
                ...
                try:
                    ...
                    if any(ne in err_s for ne in _NET_ERRS):
                        ...
                        continue  # безлімітний retry на мережу, як в brute
                    last_err = str(e)
                    break

            # fail_codes = коди які означають "не знайдено" (напр. 404)
            # все інше — знахідка
            if last_err:
                if not hide_no_ota:
                    self._ub_log(f"✗ ERR    {url}  — {last_err}", 'error')
            elif status in fail_codes:
                # пропускаємо — це fail (404 тощо)
                if not hide_no_ota:
                    self._ub_log(f"  [{status}]  {url}", 'skip')
            else:
                # будь-який інший статус = знахідка
                with self._ub_lock:
                ...
                if stop_on_find and not is_dup:
                    self._ub_pause_event.clear()   # блокує всі воркери одразу
                    self.root.after(0, self._ub_pause_on_find)  # тільки UI

            self._ub_increment_progress()

    def _ub_pause_on_find(self):
        """Called on the main thread when 'Stop on first found' triggers — pauses instead of stopping."""
        if not self._ub_running:
            return
        self.ub_pause_btn.config(state=tk.DISABLED)
        self.ub_continue_btn.config(state=tk.NORMAL)
    ...
    def _build_devbrute_tab(self):
        ...
        def _bind_mw(w):
            ...
            for c in w.winfo_children(): _bind_mw(c)
        ...
        # ── Wordlist (replaces charset brute) ─────────────────────────────
    ...
    def _db_pause(self):
        if not self._devbrute_running: return
    ...
    def _db_visual_pause(self):
        """Called on main thread when a worker auto-pauses on find."""
        if not self._devbrute_running: return
        self.db_pause_btn.config(state=tk.DISABLED)
        self.db_continue_btn.config(state=tk.NORMAL)
    ...
    def _db_update_speed(self):
        """Called periodically on main thread to refresh the req/s counter."""
    ...
    def _db_start(self):
        ...
        if not key_list:
            key_list = [""]  # single empty key if template has no {key}
        ...
        # Total combinations
        if use_wordlist:
            total = 0  # unknown until wordlist is streamed; progress bar in indeterminate-style
        elif fixed:
    ...
    def _db_producer_wordlist(self, wordlist_path, key_list, n_workers):
        """Producer that feeds words from a file instead of charset combos."""
        try:
            for word in self._db_wordlist_iter(wordlist_path):
                ...
                # Update total count dynamically so progress bar grows
                with self._devbrute_lock:
    ...
    def _db_worker(self, template, locale, stop_on_find, device_sn="", imei=""):
        ...
        while True:
            ...
            # Build fingerprint from template
            fp = template
            if '{device}' in fp:
                fp = fp.replace('{device}', device)
            if '{key}' in fp:
            ...
            while True:
                ...
                try:
                    ...
                    is_network = any(ne in err_str for ne in _NETWORK_ERRS)
                    if is_network:
                        continue  # retry unlimited on network errors
                    hard_attempt += 1
                    if hard_attempt >= max_hard_retries:
                        self._db_log(
    ...
    # ── Query / Key scan core ─────────────────────────────────────────────
    _CHECKIN_NETWORK_ERRS = ("handshake operation timed out",
                             "read operation timed out",
                             "_ssl.c",
                             "urlopen error",
                             "timed out",
    ...
    def _checkin_with_retry(self, fp, status_cb=None, **kwargs):
        """Call perform_checkin with the same retry policy as the bruteforce
        workers: transient network errors retry indefinitely (bounded), empty
        responses retry up to max_hard_retries.  One-shot GUI paths (Query /
        Key scan / OTA Chain) previously made a single attempt and reported
        every transient failure as an error — that is why bruteforce found
        updates where a single check-in 'sometimes' failed.  The single query
        additionally uses a dedicated connection (use_pool=False) so it can
        never inherit a stale keep-alive socket left under a recycled thread
        id by dead bruteforce workers.

        Returns (settings, raw_bytes, req_data, req_gz, last_error).
        """
        max_hard_retries = kwargs.pop('max_hard_retries', 3)
        max_net_retries = kwargs.pop('max_net_retries', 3)
        ...
        while True:
            try:
                settings, raw_bytes, req_data, req_gz = perform_checkin(fp, **kwargs)
                if not settings:
                    last_error = "No response from server"
                    hard_attempt += 1
                    if status_cb:
                        status_cb(f"  ⚠ Empty response — retrying ({hard_attempt}/{max_hard_retries})…")
                    if hard_attempt < max_hard_retries:
                        continue
                    return None, raw_bytes, req_data, req_gz, last_error
                return settings, raw_bytes, req_data, req_gz, None
            except ValueError:
                raise  # bad fingerprint — never retry
            except Exception as e:
                err_str = str(e).lower()
                is_network = any(ne in err_str for ne in self._CHECKIN_NETWORK_ERRS)
                last_error = str(e)
                if is_network:
                    net_attempt += 1
                    if status_cb:
                        status_cb(f"  ⚠ Network error ({e}) — retrying ({net_attempt}/{max_net_retries})…")
                    if net_attempt < max_net_retries:
                        continue
                return None, None, None, None, last_error

    def _is_main_thread(self):
        return threading.get_ident() == getattr(self, '_root_main_thread', threading.get_ident())

    def _post(self, func):
        """Run a tk-callback on the main thread (or directly if already there)."""
    ...
    def update_status(self, message, status_type='info'):
        # Thread-safe: Query / keyscan / chain run these from worker threads;
        # tkinter must only be touched from the mainloop thread.
        def _apply():
    ...
    def _make_scrollable_frame(self, parent):
        ...
        def _on_mousewheel(event):
            try:
                ...
                # canvas was destroyed (window closed); unbind to stop future calls
                try:
        ...
        def worker():
            while True:
                ...
                try:
                    _curl = getattr(self, 'checkin_url_var', None)
                    _curl = _curl.get().strip() if _curl else None
                    settings, raw_bytes, _req, _reqgz, _err = self._checkin_with_retry(
                        test_fp, locale=loc, timezone=tz, device_sn=device_sn, imei=imei, url=_curl
                    )
                    if not settings:
                        error_msg = _err or "No response from server"
                    else:
                        ota = find_ota_link(settings)
    ...
    def _update_dogfood_label(self, detected: bool):
        if detected:
    ...
    def on_query_click(self):
        mode = self.os_mode_var.get()
        if mode == "playemu":
            # Play Games Emulator has its own send button; the main Query button
            # just proxies to it without needing a fingerprint.
            self._playemu_send()
            return
    ...
    def perform_query(self, fingerprint):
        ...
        try:
            ...
            # Same retry policy as the bruteforce workers — transient network
            # blips and empty responses no longer surface as a hard "Query failed".
            settings, raw_bytes, req_data, req_gz, last_err = self._checkin_with_retry(
                fingerprint, status_cb=lambda m: self.update_status(m.strip()),
                locale=locale, timezone=timezone, device_sn=device_sn, imei=imei,
                url=checkin_url)
            ...
            self._post(self._raw_request_populate)

            dogfood = False
            if raw_bytes and (b'droidfood' in raw_bytes.lower() or b'platform_dogfood' in raw_bytes.lower()):
                dogfood = True
            self._post(lambda: self._update_dogfood_label(dogfood))

            if not settings:
                self.log_output(f"ERROR: Check-in failed - {last_err or 'No response from server'}", 'error')
                self._post(lambda: self.status_icon_var.set("❌"))
                ...
                self._post(lambda: self._raw_populate(human_dump, hex_dump))

                build_info = extract_build_details(fingerprint, settings)
                ota_link = find_ota_link(settings)

                # Downgrade classification: served OTA targets a build lower
                # than the claimed fingerprint → genuine signed downgrade OTA
                if ota_link and ota_link.get('postcondition'):
                    input_key = fingerprint_sort_key(fingerprint)
                    target_key = fingerprint_sort_key(ota_link['postcondition'])
                    ota_link['is_downgrade'] = bool(
                        input_key and target_key and target_key < input_key)
                ...
                if self.json_var.get():
                    ...
                    def _render_json(out=output_str, has_ota=bool(ota_link and ota_link.get('url'))):
                        if self.html_frame:
                            self.html_frame.load_html(f"<pre>{out}</pre>")
                        elif self.desc_text:
                            self.desc_text.insert(tk.END, out)
                        if has_ota:
                            self.status_icon_var.set("✓")
                    ...
                    self._post(_render_json)
                else:
                    # Render on the main thread (format_and_log_output touches
                    # many widgets; the old direct worker-thread call could
                    # throw sporadic Tcl errors).
                    self._post(lambda: self.format_and_log_output(fingerprint, settings, build_info, ota_link, dogfood))
                ...
                if self.save_var.get():
                    if self.json_var.get():
                        ...
                        def _save_after_render(_s=_json_str):
                            self.save_output(_s, fingerprint)
                    else:
                        def _save_after_render():
                            self.save_output(self.output_text.get(1.0, tk.END), fingerprint)
                    # Posted after the render above → runs after it on the main
                    # loop (FIFO), so the saved text contains the full report.
                    self._post(_save_after_render)
            ...
            # Buttons must be re-enabled on the main thread — this runs in a
            # worker thread.
            def _restore():
    ...
    def perform_query_chromeos(self, fingerprint):
        try:
            ...
            # Fix: populate Raw Request tab for ChromeOS (XML request bytes)
            ...
            if not response_text:
                ...
                if self.json_var.get():
                    ...
                    self._post(lambda: self.format_and_log_output(fingerprint, {}, build_info, ota_link, dogfood))
                ...
                if self.save_var.get():
                    if self.json_var.get():
                        ...
                        def _save_after_render():
                            self.save_output(self.output_text.get(1.0, tk.END), fingerprint)
                    # Posted after the render above → runs after it (FIFO), so
                    # the saved text contains the full report.
                    self._post(_save_after_render)
    ...
    def perform_query_xiaomi(self, fingerprint):
        try:
            ...
            self._last_query_token = ''
            # Populate Raw Request tab with the actual URL-encoded POST body
            ...
            if not decrypted:
                ...
                if not details.get('found'):
                    ...
                    if self.json_var.get():
                        ...
                        def _render_json(out=output_str, has_ota=bool(ota_dict and ota_dict.get('url'))):
                            if self.html_frame:
                                self.html_frame.load_html(f"<pre>{out}</pre>")
                            elif self.desc_text:
                                self.desc_text.insert(tk.END, out)
                        ...
                        self._post(lambda: self.format_and_log_output(fingerprint, {}, build_info, ota_dict, dogfood))
                        if details.get('md5'):
                    ...
                    if self.save_var.get():
                        if self.json_var.get():
                            ...
                            def _save_after_render():
                                self.save_output(self.output_text.get(1.0, tk.END), fingerprint)
                        # Posted after the render above → runs after it (FIFO).
                        self._post(_save_after_render)
    ...
    def perform_query_playemu(self):
        """Send an Omaha JSON request for the Play Games Emulator app."""
        import uuid
        try:
            def _clear_ui():
                self.output_text.delete(1.0, tk.END)
            ...
            # Build request from visual editor fields
            ...
            # Strip the )]}' prefix that Google adds
            clean_text = raw_text
            ...
            # Parse the response — supports protocol 4.0 (pipelines), 3.x, 2.0 (gupdate)
            resp_data = json.loads(clean_text)
            ota_url, ota_size, ota_sha256, next_version, description_lines, other_urls =                 _parse_playemu_response(resp_data)
            ...
            if ota_url is not None:
                ...
                if self.json_var.get():
                    ...
                    def _render_json(out=output_str):
                        if self.html_frame:
                            self.html_frame.load_html(f"<pre>{out}</pre>")
                        elif self.desc_text:
                            self.desc_text.insert(tk.END, out)
                    ...
                    self._post(lambda: self.format_and_log_output("", {}, build_info, ota_link, False))

                if self.save_var.get():
                    if self.json_var.get():
                        _json_str = output_str

                        def _save_after_render(_s=_json_str):
                            self.save_output(_s, "playemu")
                    else:
                        def _save_after_render():
                            self.save_output(self.output_text.get(1.0, tk.END), "playemu")
                    # Posted after the render above → runs after it (FIFO).
                    self._post(_save_after_render)
                ...
                # No update — still show success (server responded fine, just no OTA)
    ...
    def _build_playemu_editor(self, parent):
        """Build compact inline Play Games Emulator Omaha visual editor."""
        ...
        # Initialise all StringVar/BooleanVar
        ...
        # Row 1: @os, @updater, acceptformat, arch, dedup
        r1 = ttk.Frame(parent)
        ...
        # Row 2: prodversion, updaterversion, protocol
        r2 = ttk.Frame(parent)
        ...
        # Row 3: appid, ap, version, enabled, installsource
        r3 = ttk.Frame(parent)
        ...
        # Row 4: installdate, installsource, ping.rd
        r4 = ttk.Frame(parent)
        ...
        # Row 5: os.platform, os.arch, os.version, physmemory
        r5 = ttk.Frame(parent)
        ...
        # Row 7: reset button (send is via main Query button)
        r7 = ttk.Frame(parent)
    ...
    def _build_playemu_tab(self):
        """Legacy stub — editor moved to main area."""
        pass
    def _playemu_send(self):
        """Trigger a Play Games Emulator Omaha query from the visual tab."""
        self.query_button.config(state=tk.DISABLED)
        self.keyscan_button.config(state=tk.DISABLED)
        self.clear_button.config(state=tk.DISABLED)
        self.fingerprint_entry.config(state=tk.DISABLED)
    ...
    def _playemu_reset(self):
        """Reset all Play Games Emulator fields to defaults."""
    ...
    def _build_rawreq_tab(self):
        ...
        # ── Scrollable canvas for top controls ───────────────────────────
        ...
        # Not packed by default (text mode is default)
        ...
        # ── Response area (grows to fill remaining space) ─────────────────
        # Placed OUTSIDE the canvas/inner so it always fills the bottom half
        ...
        # Action buttons above the notebook
    ...
    def _rawreq_worker(self, url, raw_bytes, is_xml):
        try:
            if is_xml:
                ...
                # Checkin: gzip-compress if not already
    ...
    def _rawreq_display_xml(self, resp_bytes, resp_text, resp_headers, sent_bytes):
        ...
        # Try to extract OTA via ChromeOS parser
        try:
    ...
    def on_stop_scan_click(self):
        """Unified stop — signals both key scan and OTA chain check to halt."""
    ...
    def perform_keyscan(self, fingerprint):
        try:
                ...
                for key in key_types:
                    ...
                    try:
                        ...
                        settings, raw_bytes, _req, _reqgz, _err = self._checkin_with_retry(
                            test_fp, locale=loc, timezone=tz, device_sn=device_sn, imei=imei, url=_curl)
                        if not settings:
                            self.log_output(f"  Status: ❌ {_err or 'No response from server'}", 'error')
                            continue
            ...
            self._post(lambda: self.status_icon_var.set("✓" if found_links else "❌"))
    ...
    def perform_otachain(self, start_fingerprint):
        try:
            ...
            # Switch to Full Log tab so the user sees output immediately
            try:
            ...
            # Token used for subsequent checks (payload metadata etc.).
            # Seeded manually from the HTTP Info "Token" field, then updated
            # from each checkin response's update-token (if present).
            try:
                token = getattr(self, 'httpinfo_token_var', tk.StringVar()).get().strip()
            except Exception:
                token = ""
            ...
            if token:
                self.log_output(f"Auth token           : {token[:16]}… (will be reused for subsequent checks)", 'info')
            self.log_output("", 'info')

            visited_fingerprints = []   # ordered list of fingerprints checked
            visited_urls = set()        # set of OTA URLs already seen in this chain
            chain_steps = []            # list of (fingerprint, ota_url, post_build)
            ...
            while True:
                ...
                # Guard against fingerprint loop
                ...
                # --- Perform checkin (with bruteforce-grade retries) ---
                try:
                    settings, raw_bytes, _req, _reqgz, _err = self._checkin_with_retry(
                ...
                if not settings:
                    self.log_output(f"  ❌ {_err or 'No response from server'} — chain ends here.", 'error')
                    termination_reason = "no response from server"
                    break

                # Capture update-token from the checkin response — it will be
                # reused (Authorization header) for all subsequent checks.
                try:
                    resp_token = (settings.get('update_token') or '').strip()
                except Exception:
                    resp_token = ''
                if resp_token and resp_token != token:
                    token = resp_token
                    self.log_output(f"  🔑 Update-token received: {token[:16]}… — will be used for subsequent checks", 'success')
                ...
                # --- Fetch payload metadata to get post-build fingerprint ---
                if self._otachain_stop_event.is_set():
                    stopped = True
                    termination_reason = "stopped by user"
                    chain_steps.append((current_fp, ota_url, None))
                ...
                self.log_output(f"  🔍 Fetching payload metadata to extract post-build fingerprint{' (using token)' if token else ''}…", 'info')
            ...
            self._post(lambda: self.status_icon_var.set("✓" if found_count else "❌"))
...
def _parse_playemu_response(resp_data):
    """
    Parse an Omaha JSON response for the Play Games Emulator app.
    Supports protocol 4.0 (pipelines inside response.apps[]),
    protocol 3.x (response.app[] with manifest/urls structure),
    and protocol 2.0 (gupdate.app[] with flat updatecheck fields).
    Returns: (ota_url, ota_size, ota_sha256, next_version, description_lines, other_urls)
    ota_url is None if no update was found.
    """
    ota_url = None
    ota_size = ""
    ota_sha256 = ""
    ...
    # Determine the root key: "response" (4.0/3.x) or "gupdate" (2.0)
    root = resp_data.get("response") or resp_data.get("gupdate") or {}

    # Normalise apps list — protocol 4.0 uses "apps", 3.x/2.0 use "app"
    ...
    for app in apps:
        ...
        # ── Protocol 3.x: manifest + urls ──────────────────────────────
            ...
            # Collect codebases from urls.url[]
            codebases = []
            ...
            # Choose best codebase (prefer https)
            chosen_base = None
            for cb in codebases:
            ...
            # Package name from packages.package[]
            ...
            # Fallback filename from manifest.run or actions.action[event=install].run
            if not pkg_name:
        ...
        # ── Protocol 2.0: flat updatecheck fields ──────────────────────
        else:
            # codebase is the full URL
            cb = uc.get("codebase", "")
            if cb:
                ota_url = cb
...
def _iter_actions(manifest):
    """Yield action dicts from manifest.actions.action (handles list or dict)."""
...
def perform_checkin_chromeos(fingerprint, app_id, arch="x86_64", hardware_class=None):
    # Network/SSL errors propagate to the caller (brute worker handles retries)
...
# ── HTTP/1.1 keep-alive connection pool for Android checkin ─────────────────
# One persistent HTTPConnection per (host, port) per thread — no SSL overhead,
# no TCP handshake on every request.  Gives maximum bruteforce throughput.
import threading as _threading
_checkin_pool_lock = _threading.Lock()
_checkin_pool: dict = {}   # key: (host, port, is_https, proxy_key, thread_id) → HTTPConnection
# One mutex per pooled connection.  Thread-id keys are recycled by the OS once
# a thread dies, so a freshly spawned thread can inherit another thread's pool
# entry (e.g. a single Query while a bruteforce is still running).  Without a
# per-connection lock, two live threads would pipeline requests onto the same
# socket and interleave responses.
_conn_locks: list = []     # [(weakref.ref(conn), lock)]


def _conn_lock_for(conn):
    """Return the per-connection send mutex (creating it on first use)."""
    import weakref
    with _checkin_pool_lock:
        for i in range(len(_conn_locks) - 1, -1, -1):
            ref, lk = _conn_locks[i]
            c = ref()
            if c is None:
                _conn_locks.pop(i)   # connection is gone — drop its mutex
            elif c is conn:
                return lk
        lk = _threading.Lock()
        _conn_locks.append((weakref.ref(conn), lk))
        return lk


def _get_proxy_for_url(url):
    """Get proxy settings for a given URL using urllib's proxy detection."""
    try:
        proxies = urllib.request.getproxies()
        parsed = urlparse(url)
        scheme = parsed.scheme.lower()
        return proxies.get(scheme) or proxies.get(scheme.upper())
    except Exception:
        return None


def _get_checkin_conn(host, port, is_https, timeout=10, proxy_url=None, pooled=True):
    """Return a cached keep-alive connection, or create one if needed.

    With pooled=False a fresh throw-away connection is returned every time —
    used by the one-shot Query path so it can never inherit a stale socket
    left under a recycled thread id by dead bruteforce workers.
    """
    key = None
    if pooled:
        proxy_key = proxy_url or "none"
        key = (host, port, is_https, proxy_key, _threading.get_ident())
        with _checkin_pool_lock:
    ...
    if is_https:
        ...
        if proxy_url:
            # For HTTPS through proxy, connect to proxy and use CONNECT tunnel
            proxy_parsed = urlparse(proxy_url)
            proxy_host = proxy_parsed.hostname
            proxy_port = proxy_parsed.port or 8080
            conn = http.client.HTTPSConnection(
                proxy_host, proxy_port, timeout=timeout, context=_ssl_ctx
            )
            tunnel_headers = {}
            if proxy_parsed.username or proxy_parsed.password:
                import base64
                auth = f"{proxy_parsed.username or ''}:{proxy_parsed.password or ''}"
                tunnel_headers['Proxy-Authorization'] = 'Basic ' + base64.b64encode(auth.encode()).decode()
            conn.set_tunnel(host, port, headers=tunnel_headers or None)
        else:
            conn = http.client.HTTPSConnection(host, port, timeout=timeout, context=_ssl_ctx)
    else:
        if proxy_url:
            proxy_parsed = urlparse(proxy_url)
            conn = http.client.HTTPConnection(proxy_parsed.hostname, proxy_parsed.port or 80, timeout=timeout)
        else:
    ...
    if pooled:
        with _checkin_pool_lock:
            ...
            # Purge entries owned by threads that no longer exist.  Thread ids
            # get recycled, and a dead worker's socket would otherwise leak in
            # the pool forever (and be handed to the next thread reusing that id).
            alive_ids = {t.ident for t in _threading.enumerate()}
            for stale_key in [k for k in _checkin_pool if k[-1] not in alive_ids]:
                stale_conn = _checkin_pool.pop(stale_key)
                try:
                    stale_conn.close()
                except Exception:
                    pass
...
def perform_checkin(fingerprint, locale="en-US", timezone="America/New_York", device_sn="", imei="", url=None, use_pool=True):
    # Network/SSL errors propagate to the caller (brute worker handles retries).
    # use_pool=False: one-shot callers (single Query) get a dedicated connection
    # so they can never touch a stale socket left under a recycled thread id.
    parsed = parse_fingerprint(fingerprint)
    ...
    # Detect proxy from environment/system settings
    ...
    # For HTTP through proxy, use absolute URL in request line
    if proxy_url and not is_https:
        request_path = url
    else:
        request_path = path

    # Try up to 2 times in case the pooled connection was closed server-side
    for attempt in range(2):
        conn = _get_checkin_conn(host, port, is_https, timeout=10,
                                 proxy_url=proxy_url, pooled=use_pool)
        # Serialize request/response on the socket: a pooled entry can be seen
        # by two live threads when an OS thread id gets recycled mid-run.
        _local_lock = _threading.Lock()
        _lock = _conn_lock_for(conn) if use_pool else _local_lock
        try:
            with _lock:
                conn.request('POST', request_path, body=compressed, headers=request_headers)
                resp = conn.getresponse()
                status = resp.status
                response_data = resp.read()
        except Exception:
            # Connection was stale — close and retry with a fresh one
            try:
                conn.close()
            except Exception:
                pass
            if use_pool:
                key = (host, port, is_https, proxy_url or "none", _threading.get_ident())
                with _checkin_pool_lock:
            ...
            continue
        if status >= 400:
            # Never feed an error page into the protobuf parser — surface it as
            # an error so the caller's retry logic can take over.
            try:
                conn.close()
            except Exception:
                pass
            if use_pool:
                ...
                with _checkin_pool_lock:
                    _checkin_pool.pop(key, None)
            if attempt == 1:
                raise IOError(f"HTTP {status} from checkin server")
            continue
        break
    if not use_pool:
        try:
...
def _safe_int(s):
    ...
    try:
        ...
        pass
    # Non-integer values like '5.1' (api level) → fall back to the major
    # version number so cross-version comparisons still sort correctly.
    major = s.split('.')[0] if s else ''
    try:
        return int(major)
    except ValueError:
        return 0
...
def fingerprint_sort_key(fp):
    """
    Rough monotonic ordering key for Android fingerprints:
    (api_level, incremental). Returns None if unparseable.
    Used to tell whether a served OTA targets a build lower than the
    claimed one (i.e. an actual downgrade).
    """
    try:
        p = parse_fingerprint(fp)
    except Exception:
        return None
    return (_safe_int(p.get('api_level')), _safe_int(p.get('incremental')))
...
def _parse_ota_suffix_core(core: str):
    ...
    Handles the variant where a trailing '.XXXXXXXX' 8-char short hash is
    appended after the PRE build id (e.g. 'tervigon-HMJ37-from-HWI69.e379f0b9'
    or 'trygon-JZO54M-from-IMM76L.bfd5cc10') by stripping it before matching.
    """
    # Strip a trailing '.XXXXXXXX' short hash (8 hex chars) that some older
    # OTA filenames append after the pre-build id (e.g. .e379f0b9).
...
def probe_direct_url_alternate(url: str, token: str = "", status_cb=None, timeout: int = 12, max_workers: int = 40):
    ...
    if sha1:
        ...
        if is_signed_family:
            ...
            # Also generate candidates using the first 12 hex chars of the
            # sha1 as the stem (e.g. 19dea2ba1ae5.signed-trygon-...).
...
def _build_id_from_fingerprint(fingerprint: str):
    """Return (build_tag, device, product) from a raw fingerprint string,
    or (None, None, None) if it doesn't parse."""
    try:
        parsed = parse_fingerprint(fingerprint)
...
def build_alternative_filenames(sha1: str, device: str, post_build_id: str,
    ...
    # 12-char prefix seen in older OTA filenames, e.g.:
    #   19dea2ba1ae5.signed-trygon-HTJ85B-from-HMJ37.19dea2ba.zip
    # where the first 12 hex chars of the full sha1 are used as the file
    # stem, followed by the descriptive suffix and a trailing 8-char short.
    ...
    if pre_build_id and post_build_id:
        ...
        # 12-char sha1 prefix variants (e.g. 19dea2ba1ae5.signed-trygon-...-from-....19dea2ba.zip)
        if short12 != sha1:
        ...
        # signed-signed- with _full_radio suffix
        ...
        # re-signed- (dash, not underscore) family
        ...
        # signed-two-step.signed- family
        # short = first 8 chars, short_end = last 8 chars of sha1
    ...
    # Re-release variant: every candidate generated above is also tried
    # with '1' appended immediately before the trailing '.zip', e.g.:
    #   sha1.signed-device-POST-from-PRE.zip
    #   -> sha1.signed-device-POST-from-PRE1.zip
...
def probe_alternative_filenames(url: str, last_modified: str,
    ...
    if any(marker in original_fname for marker in _longform_markers) or \
        ...
        # Long-form filename — fall through to the full pattern generation
        # below so that all alternative names are checked, not just the
        # trimmed sha1.zip variant.
        pass
    ...
    if not device:
        # Try to extract device/post/pre directly from the filename itself
        # (works when a long-form URL is pasted but fingerprints are empty).
        _fname_sha1, _fname_suffix = _split_ota_longform_filename(original_fname)
        if _fname_suffix:
            _core = _fname_suffix
            for _pfx in ('loose_ota-signed-', 'signed-signed-', 'signed_signed-',
                         'signed-unsigned-', 're_signed-', 'Dotasigned-', 'signed-'):
    ...
    if not device:
        ...
        # independent candidates (e.g. sha1.zip, sha1.update.zip).
        ...
        candidates = []
        seen_candidates = set()
        # sha1 variants to try as filename stems: the full sha1 (standard)
        # and the first 12 hex chars (seen on very old OTAs, e.g.
        # 19dea2ba1ae5.signed-trygon-HTJ85B-from-HMJ37.19dea2ba.zip).
        sha1_stems = [sha1]
        if sha1 and len(sha1) >= 12:
            sha1_12 = sha1[:12]
        ...
        # devkeys-to-releasekeys variant: when pre-build uses dev-keys and
        # post-build uses release-keys, e.g.:
        # 4f2128c0...-razor-devkeys-to-releasekeys-JWR66N.zip
        pre_key = ''
        post_key = ''
        try:
...
def _create_proxy_aware_conn(host, port, is_https, timeout, proxy_url=None, target_host=None, target_port=None):
    """Create an http.client connection with optional proxy support."""
    if is_https:
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        if proxy_url:
            ...
            if proxy_parsed.username or proxy_parsed.password:
                ...
                auth = f"{proxy_parsed.username or ''}:{proxy_parsed.password or ''}"
                tunnel_headers['Proxy-Authorization'] = 'Basic ' + base64.b64encode(auth.encode()).decode()
            conn.set_tunnel(target_host or host, target_port or port, headers=tunnel_headers or None)
        else:
...
def probe_ota_url(url: str, token: str = "", status_cb=None, timeout: int = 15) -> dict:
    ...
    for hop in range(max_redirects):
        ...
        # Detect proxy for this hop's URL
        proxy_url = _get_proxy_for_url(current_url)
        ...
        try:
            ...
            conn = _create_proxy_aware_conn(host, port, is_https, timeout, proxy_url, host, port)
            ...
            # For HTTP through proxy, use absolute URL
            request_path = current_url if (proxy_url and not is_https) else path

            conn.request('HEAD', request_path, headers=req_headers)
            resp = conn.getresponse()
